...
function reverseNumeberFormat(num){
...
for( var i = 0; i < operator.length; i++){
    operator[i].addEventListener('click',function(){
        if(this.id=="clear"){
            ...
            var
            output=reverseNumeberFormat(getOutput()).toString();
        ...
        if(output!=""|| history!=""){
            output = output==""?
            output:reverseNumeberFormat(output);
...
for(var i=0; i<number.length; i++){
    number[i].addEventListener('click',function(){
        var output=reverseNumeberFormat(getOutput());
        if(output!=NaN){
